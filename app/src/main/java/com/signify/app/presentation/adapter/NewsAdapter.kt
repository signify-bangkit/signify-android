...
class NewsAdapter(private val context: Context, private val newsList: List<News>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NewsViewHolder {
        val binding = ListArticleBinding.inflate(LayoutInflater.from(context), parent, false)
    ...
    inner class NewsViewHolder(private val binding: ListArticleBinding) :
        ...
        fun bind(newsItem: News) {
            ...
            binding.ivItemNews.load(newsItem.imageUrl) {
                transformations(RoundedCornersTransformation(25f))
            }

            // Set click listener
            binding.root.setOnClickListener {
                val action = HomeFragmentDirections.actionHomeFragmentToArticleDetailFragment(
                    newsItem.imageUrl
                )
