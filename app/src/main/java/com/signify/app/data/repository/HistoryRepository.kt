...
interface HistoryRepository {
    ...
    fun deleteHistory(history: String): Flow<ApiStatus<ApiResponse>>
}
...
class HistoryRepositoryImpl(private val api: SignifyService) :
    ...
    override fun deleteHistory(history: String): Flow<ApiStatus<ApiResponse>> = flow {
        try {
            emit(ApiStatus.Loading)
            val response = api.deleteHistory(history)
            if (!response.error) {
                emit(ApiStatus.Success(response))
            } else {
